...
def compute_feature_significance(
    ...
    for col in feature_cols:
        ...
        _, p_value = ttest_ind(x1, x0, equal_var=False, nan_policy="omit")
...
def plot_top_feature_importance(
    ...
    """
    Plot top feature importance results for the three tuned models:
      - Logistic Regression absolute coefficients
      - Random Forest feature importances
      - HistGradientBoosting absolute permutation importances

    If consensus_df is provided, use the consensus ranking where possible so
    the panels are easier to compare across models.
    """
    model_specs = [
        (
            "logistic_regression",
        ...
        (
            "hist_gradient_boosting",
            "abs_permutation_importance_mean",
    ...
    if consensus_df is not None and not consensus_df.empty and "feature" in consensus_df.columns:
        consensus_order = consensus_df["feature"].tolist()

    for ax, (model_key, value_col, title) in zip(axes, model_specs):
        df_imp = importance_outputs.get(model_key)

        if df_imp is None or df_imp.empty or value_col not in df_imp.columns:
            ax.text(0.5, 0.5, f"No data for {model_key}", ha="center", va="center", fontsize=12)
            ax.set_title(title)
            ax.axis("off")
            continue
        ...
        if consensus_order is not None:
            selected = [f for f in consensus_order if f in plot_df["feature"].values][:top_n]

            if len(selected) < top_n:
                extras = [
                    f for f in plot_df.sort_values(value_col, ascending=False)["feature"].tolist()
                    if f not in selected
                ]
                selected.extend(extras[: top_n - len(selected)])

            plot_df = plot_df[plot_df["feature"].isin(selected)].copy()
        ...
        ax.barh(plot_df["feature"], plot_df[value_col], alpha=0.85)
        ax.set_title(title)
        ...
        offset = 0.01 * max_val if max_val > 0 else 0.001

        for i, v in enumerate(values):
            ax.text(v + offset, i, f"{v:.3f}", va="center", fontsize=9)

    plt.suptitle("Top feature importance across tuned models", fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.96])

    if out_path:
        plt.savefig(out_path, dpi=150, bbox_inches="tight")
        print(f"Saved top feature importance plot to: {out_path}")
    plt.show()
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="How many features to show per model in the top feature importance figure.",
    )
    ...
    top3_features = consensus_df["feature"].head(3).tolist() if not consensus_df.empty else []
    ...
    if stats_df.empty:
        print("No valid significance tests could be computed.")
    else:
        print(stats_df.head(15).to_string(index=False))

    print("\nConsensus top 10 features:")
    if consensus_df.empty:
        print("No consensus ranking could be computed.")
    else:
    ...
    if top3_features and not stats_df.empty:
    ...
    plot_importance = f"{args.save_plots_prefix}_feature_importance.png" if args.save_plots_prefix else None
